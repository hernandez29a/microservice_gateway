...
import { FlightMSG, PassengerMSG } from '../common/constanstst';
...
export class FlightController {
    ...
    @Put(':id')
    update(@Param('id') id: string, @Body() fligDTO: FlightDTO): Observable<IFlight>{
        return this._clientProxyFlight.send(FlightMSG.UPDATE, {id, fligDTO});
    }
    ...
    async AddPassenger( @Param('flightId') flightId: string , @Param('passengerId') passengerId: string  ){
        ...
        if(!passenger) throw new HttpException('No hay un pasagero con ese Id', HttpStatus.NOT_FOUND);
